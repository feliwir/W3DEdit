...
// Module : W3dModelFile.h
// Author : Level_3D_Vision (C) 2008-2012
// System : ANSI C/C++ - MinGW32 compiler
// -------------------------------------------------------------------------------
// History: 03.11.09 -- adapted and changed by Jerry Brutzki
//          01.02.11 -- changed by Jerry Brutzki
//          30.10.14 -- changed by Stephan Vedder
//********************************************************************************
// Data Structures for W3D files
//--------------------------------------------------------------------------------
// Originally provided by Westwood Studios, some modifications to make
// things compile in C done by Jonathan Wilson
//
...
//********************************************************************************
#ifndef W3DMODELFILE_INCLUDED_H
#define W3DMODELFILE_INCLUDED_H

//********************************************************************************
// NAMING CONVENTIONS:
//    Typical render object name is 15 characters + NULL
//    Meshes have 31 + NULL character name formed from the concatenation of the
//    "container" model name and the mesh's name: "ContainerName.MeshName"
//
//    Animations have 31 + NULL character names formed from the concatenation of
//    the Hierarchy tree name with the animation name: "AnimationName.HierarchyName"
...
//--------------------------------------------------------------------------------
// November 3, 2009  (JWB)
//   Renamed and Modified the original file as copied from the internet to fit into
//   the new class design of CW3dModelBase. All necessary type definitions are included.
//   These definitions were transferred from the header file bittype.h.
//   For reading BFME-Models, I added the following chunks:
//       W3D_CHUNK_VERTICES_COPY,
//       W3D_CHUNK_VERTEX_NORMALS_COPY
//
//--------------------------------------------------------------------------------
// March 31, 2011  (JWB)
//   Introduced new value W3D_TYPE_LEN and new flags in order to accomodate the
//   reading of structures for NormalMap material/textures in BFME2:
//
//   W3D_CHUNK_NORMALMAP_INFO       =0x00000050,
//       W3D_CHUNK_NORMALMAP_FLAG1  =0x00000051,
//       W3D_CHUNK_NORMALMAP_FLAG2  =0x00000052,
//       W3D_CHUNK_NORMALMAP_FLAG3  =0x00000053,
//
//   W3dNormMapHeaderStruct, W3dNormMapEntryStruct
//
//--------------------------------------------------------------------------------
// April 10, 2011  (JWB)
//   Introduced new flags in order to accomodate the reading of extra structures
//   for animations in BFME2:
//
//   W3D_CHUNK_COMPRESSED_ANIMATION_BFME2DATA,
//   W3dTimeCodedAnimVectorStruct
//********************************************************************************
...
#define WWMATH_FLOAT_MAX 3.40282347E+38
...
//********************************************************************************
//  W3D-FORMAT: CHUNK TYPES DEFINITIONS
//--------------------------------------------------------------------------------
...
enum
{
    ...
    W3D_CHUNK_VERTICES_COPY = 0x00000C00,  // array of vertices (array of W3dVectorStruct's for BFME)
    W3D_CHUNK_VERTEX_NORMALS = 0x00000003,  // array of normals  (array of W3dVectorStruct's)
    W3D_CHUNK_VERTEX_NORMALS_COPY = 0x00000C01,  // array of normals  (array of W3dVectorStruct's for BFME)
    ...
    W3D_CHUNK_NORMALMAP_INFO = 0x00000050,  // material/texture 1st marker flag for BFME2 (???)
    W3D_CHUNK_NORMALMAP_FLAG1 = 0x00000051,  // material/texture 2nd marker flag for BFME2 (???)
    W3D_CHUNK_NORMALMAP_FLAG2 = 0x00000052,  // material/texture structure  of W3dNormMapHeaderStruct for BFME2
    W3D_CHUNK_NORMALMAP_FLAG3 = 0x00000053,  // array of texture structures of W3dNormMapEntryStruct for BFME2
    ...
    W3D_CHUNK_COMPRESSED_ANIMATION_BFME2DATA,                    // compressed animation data for BFME2 ??? dependent on type of compression
...
//--------------------------------------------------------------------------------
// Define: vector,quaternion,coordinate
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// DEfine: rgb color, one byte per channel, padded to an even 4 bytes
//--------------------------------------------------------------------------------
typedef struct
{
    uint8      R;
...
// ------------------------------------------------------------------------------------
// W3dMeshHeader3Struct structure definitions ...
// ------------------------------------------------------------------------------------
typedef struct
{
    uint32  Version;
...
// Vertex Influences;i.e. each vertex can be associated with a different bone.
typedef struct
{
    uint16            BoneIdx;
    uint16            XtraIdx;   // BFME models allow for 2 bones -- JWB
    uint8             Pad[4];
} W3dVertInfStruct;
...
//
//  For BFME2 the chunk W3DVERTMAT_COPY_SPECULAR_TO_DIFFUSE marks the beginning of
//  the subsequent array of texture UVWs --> W3D_CHUNK_STAGE_TEXCOORDS (JWB)
//********************************************************************************
typedef struct
{
...
// New header type structure for BFME2 ....
typedef struct
{
    uint8    Number;
    char     TypeName[W3D_TYPE_LEN];
    uint32   Reserved;
} W3dNormMapHeaderStruct;
...
typedef struct
{
    uint32   TypeFlag;
    uint32   TypeSize;
    char     InfoName[W3D_NAME_LEN];
    uint32   ItemSize;
    char     ItemName[W3D_TYPE_LEN];
} W3dNormMapEntryStruct;

// New entry section values for BFME2 ....
enum W3D_NORMALMAP_HEADER_TYPES
{
...
//--------------------------------------------------------------------------------
// W3dShaderStruct bits. These control every setting in the shader. Use the helper
// functions to set them and test them more easily.
//--------------------------------------------------------------------------------
enum W3D_SHADER_SETTINGS
{
...
{
    PSS_SRC = 0, PSS_DEST, PSS_ZERO,
    PSS_SRC_ALPHA = 0, PSS_DEST_ALPHA, PSS_ONE,
...
//--------------------------------------------------------------------------------
// Texture Animation parameters: May occur inside a texture chunk if its needed
//--------------------------------------------------------------------------------
...
//********************************************************************************
//  W3D-FORMAT: WHT (Westwood Hierarchy Tree)
//--------------------------------------------------------------------------------
...
//********************************************************************************
//  W3D-FORMAT: WHA (Westwood Hierarchy Animation)
//--------------------------------------------------------------------------------
...
// New structure for BFME2 - April 10, 2011  (JWB)
typedef struct
{
    uint16    MagicNumber;          // Purpose unknown so far ...
    uint8     VectorLen;            // num Channels
    uint8     Flags;                // channel type
    uint16    NumTimeCodes;         // number of time coded entries
    uint16    Pivot;                // pivot affected by this channel
} W3dTimeCodedAnimVectorStruct;


//********************************************************************************
//  W3D-FORMAT: HMORPHANIMCLASS
//--------------------------------------------------------------------------------
...
//********************************************************************************
//  W3D-FORMAT: PARTICLE EMITTERS
//--------------------------------------------------------------------------------
//  The following structs are used to define emitters in the w3d file.
//********************************************************************************
#define W3D_CURRENT_EMITTER_VERSION   0x00020000

// This enum contains valid defines for the Type field of the W3dEmitterUserInfoStruct.
// The programmer can add entries here that their specific loader can switch on to
// determine what type the emitter is.
// NOTE: Please add a string to the EMITTER_TYPE_NAMES array when you add an entry
// to the enum.
...
//********************************************************************************
//  W3D-FORMAT: HLod (Hierarchical LOD Model)
//--------------------------------------------------------------------------------
...
//
//  Orientation:
//  This is an oriented 3D box. It is aligned with the coordinate system it is in.
//  So its extents always point along the local coordinate system axes.
//********************************************************************************
...
//********************************************************************************
//  W3D-FORMAT: SOUND RENDER OBJECTS
//--------------------------------------------------------------------------------
...
//The following is not part of the original Software
//
//added by Stephan Vedder, parts extracted from Jerry Brutzki's W3dModelMesh.h
//Date: 30.10.2014
...
typedef struct
{
    ...
    W3dRGBAStruct*      dig;
    W3dRGBAStruct*      scg;
    W3dTextureStageStruct textureStage;
} W3dMaterialPassStruct;

//added by Stephan Vedder
typedef struct
{
    bool    isTypeMap;
    W3dNormMapHeaderStruct header;
    W3dNormMapEntryStruct* entries;
} W3dNormalMapStruct;

#endif // W3DMODELFILE_INCLUDED_H
//********************************************************************************
// Module:  W3dModelFile.h -- End of File
// *******************************************************************************
